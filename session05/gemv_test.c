...
double B[5*4];
double y_t[5] = {1, 1, 1, 1, 1};
double x_t[4] = {1, 1, 1, 1};
          ...
          double *y, size_t incY)
{
    ...
    if(incRowA > incColA) {     // A row major
        size_t mb = (m + DGEMV_DOTF_FUSE - 1)/DGEMV_DOTF_FUSE;
        ...
        dgemv_ulm_mat(m%DGEMV_DOTF_FUSE,n,
                      alpha,
                      &A[(mb-1)*DGEMV_DOTF_FUSE*incRowA], incRowA, incColA,
                      x, incX, 1,
                      &y[(mb-1)*DGEMV_DOTF_FUSE*incY], incY);

    } else {                    // A col major
        size_t nb = n/DGEMV_AXPYF_FUSE;
        printf("%zu\n",nb);
        for(size_t j=0; j<nb; ++j) {
            printf("%lf\n",A[j*incColA*DGEMV_AXPYF_FUSE]);
            axpyf_ulm(m, alpha,
...
{/*
    ...
    */
    ...
    initMatrix(5,4,B,1,5);
    printMatrix(5,4,B,1,5);
